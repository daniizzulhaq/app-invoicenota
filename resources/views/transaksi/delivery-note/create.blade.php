...
@section('title', 'Tambah Delivery Note')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">Tambah Delivery Note</h4>
        <a href="{{ route('delivery-note.index') }}" class="btn btn-secondary btn-sm">Kembali</a>
    </div>

    <form action="{{ route('delivery-note.store') }}" method="POST" id="form-dn">
        @csrf
        ...
        <div class="card-box mb-3">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="no_po" value="{{ old('no_po') }}" class="form-control @error('no_po') is-invalid @enderror">
                    @error('no_po')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="no_delivery_note" value="{{ old('no_delivery_note') }}" class="form-control @error('no_delivery_note') is-invalid @enderror">
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-check form-switch mt-1">
                        <input type="checkbox" name="pakai_ppn" value="1" class="form-check-input" id="pakai_ppn"
                            {{ old('pakai_ppn') ? 'checked' : '' }}>
                        <label class="form-check-label" for="pakai_ppn">Gunakan PPN</label>
                    </div>
                </div>

                <div class="col-md-4" id="wrap-ppn-persen" style="{{ old('pakai_ppn') ? '' : 'display:none;' }}">
                    <label class="form-label">PPN (%)</label>
                    <input type="number" step="0.01" min="0" max="100" name="ppn_persen"
                        value="{{ old('ppn_persen', 11) }}" class="form-control @error('ppn_persen') is-invalid @enderror">
                    @error('ppn_persen')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="catatan" rows="2" class="form-control @error('catatan') is-invalid @enderror">{{ old('catatan') }}</textarea>
                    @error('catatan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
