    ...
    <div class="card-box">
        <form action="{{ route('rekening.update', $rekening) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="perusahaan_id" class="form-select @error('perusahaan_id') is-invalid @enderror">
                    ...
                    @foreach($perusahaans as $perusahaan)
                        <option value="{{ $perusahaan->id }}" {{ old('perusahaan_id', $rekening->perusahaan_id) == $perusahaan->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama_bank" value="{{ old('nama_bank', $rekening->nama_bank) }}" class="form-control @error('nama_bank') is-invalid @enderror">
                @error('nama_bank')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="no_rekening" value="{{ old('no_rekening', $rekening->no_rekening) }}" class="form-control @error('no_rekening') is-invalid @enderror">
                @error('no_rekening')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="atas_nama" value="{{ old('atas_nama', $rekening->atas_nama) }}" class="form-control @error('atas_nama') is-invalid @enderror">
                @error('atas_nama')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
